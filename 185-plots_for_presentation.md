...
# organizing based on separate groups
...
ggplot(data=df, aes(x=link, y=abs(normalizedEnrichmentScore), fill=Direction)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1, size=16),
        axis.title.y = element_text(size=16),
        axis.text.y = element_text(size=12),
        legend.text = element_text(size=14)) + ylim(0, 2.5)
```
...
## DGE ACC protein_coding significance
...
mart = readRDS('~/data/rnaseq_derek/mart_rnaseq.rds')
res2 = as.data.frame(resIHW)
res2$nov = substr(rownames(resIHW), 1, 15)
gnames = merge(res2, mart, by.x='nov', by.y='ensembl_gene_id')
plot_volcanoHGNC(gnames, sprintf('DGE Diagnosis %s %s', subtype,
                    myregion), pCutoff = .05)
gene_ids = rownames(resIHW)[which(resIHW$padj < .05)]
hgnc_ids = gnames[which(gnames$padj < .05), 'hgnc_symbol']
plot_expressionHGNC(gene_ids, dds, '', hgnc_ids)
...
plot_volcanoHGNC = function(res, t_str, pCutoff = 0.05) {
