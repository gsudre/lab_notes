...
I figured out the issue with the outliers. I need to round up the counts BEFORE
I do any filtering operations! Otherwise, numbers that are closer to 0 than 1
become zeros after! Let's redo this:

```r
run_DTE_noPCA_SVs = function(count_matrix, samples, tx_meta, subtype,
                             alpha, BBB = FALSE, nSV = 1, add_cov=NA) {
    cat('Starting with', nrow(tx_meta), 'variables\n')
    if (is.na(subtype)) {
        keep_me = rep(TRUE, nrow(count_matrix))
    ...
    isexpr <- filterByExpr(X, design=design)
    myCounts = X[isexpr,]
    metaExpr = data.frame(TXNAME = substr(rownames(myCounts), 1, 15))
    metaExpr = merge(metaExpr, tx_meta, by='TXNAME', sort=F)
