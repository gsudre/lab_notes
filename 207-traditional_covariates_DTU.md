...
Like we did for DGE and DTE, let's round the counts before we do anything:
...
run_DTU_noPCA = function(count_matrix, samples, tx_meta, subtype, alpha,
    ...
    # keep only the remaining transcripts and their corresponding genes
    txdf.sub = tx_meta[match(rownames(X), tx_meta$transcript_id),]
    counts = data.frame(gene_id = txdf.sub$GENEID, feature_id = txdf.sub$TXNAME)
    counts = cbind(counts, X)
...
save(dtu_acc, dtu_cau, file='~/data/post_mortem/DTU_04082021_BBB_noPCA.RData')
```

I don't get any genes after filtering for pseudogenes.

Let's start summarizing the results:

```r
library(GeneOverlap)
load('~/data/post_mortem/DTU_04082021_BBB_noPCA.RData')
...
colnames(all_res) = c('subtype', 'nomPvalThresh', 'direction',
                      'caudateGenes', 'accGenes', 'overlap', 'pvalWhole',
                      'pvalDirOnly')
out_fname = '~/data/post_mortem/DTU_upDown_overlap_results_04092021.csv'
write.csv(all_res, file=out_fname, row.names=F)
```
...
colnames(all_res) = c('subtype', 'nomPvalThresh', 'direction',
                      'caudateGenes', 'accGenes', 'overlap', 'pvalWhole',
                      'pvalDirOnly')
out_fname = '~/data/post_mortem/DTUstageR_upDown_overlap_results_04092021.csv'
write.csv(all_res, file=out_fname, row.names=F)
```
...
for (myregion in c('acc', 'caudate')) {
    for (st in c('all', 'protein_coding')) {
        ...
        res_str = sprintf('WG21_DTU_%s_%s', myregion, st)
        DBs = c(sprintf('my_%s_sets', myregion))
        for (db in DBs) {
            cat(res_str, db, '\n')
            db_file = sprintf('~/data/post_mortem/%s.gmt', db)
...
for (r in c('acc', 'cau')) {
    for (st in c('all', 'protein_coding', 'lncRNA')) {
        res_str = sprintf('res = dtu_%s[["%s"]]', r, st)
        eval(parse(text=res_str))
        fname = sprintf('%s/DTU_%s_%s_BBB_annot_04092021.csv', mydir, r, st)
